...
func (d *b2bOrganizationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for {
		...
		if org != nil || res.ResultsMetadata.NextCursor == "" {
			break
		}
	...
	if org.OrganizationSlug != "" {
		data.Slug = types.StringValue(org.OrganizationSlug)
	}
	...
	if b, err := json.Marshal(org); err == nil {
		data.RawJSON = types.StringValue(string(b))
	...
	if v, ok := generic["allowed_auth_methods"]; ok {
		data.AllowedAuthMethods = setList(toStringSlice(v))
	}
	if v, ok := generic["auth_methods"]; ok {
		data.AuthMethods = types.StringValue(toString(v))
	}
	if v, ok := generic["email_allowed_domains"]; ok {
		data.EmailAllowedDomains = setList(toStringSlice(v))
	}
	if v, ok := generic["email_invites"]; ok {
		data.EmailInvites = types.StringValue(toString(v))
	}
	if v, ok := generic["email_jit_provisioning"]; ok {
		data.EmailJITProvisioning = types.StringValue(toString(v))
	}
	if v, ok := generic["sso_jit_provisioning"]; ok {
		data.SSOJITProvisioning = types.StringValue(toString(v))
	}
	if v, ok := generic["oauth_tenant_jit_provisioning"]; ok {
		data.OAuthTenantJITProvisioning = types.StringValue(toString(v))
	}
	if v, ok := generic["first_party_connected_apps_allowed_type"]; ok {
		data.FirstPartyConnectedAppsAllowedType = types.StringValue(toString(v))
	}
	if v, ok := generic["third_party_connected_apps_allowed_type"]; ok {
		data.ThirdPartyConnectedAppsAllowedType = types.StringValue(toString(v))
	}
	if v, ok := generic["rbac_email_implicit_role_assignments"]; ok {
		...
		for _, e := range arr {
			if m, ok := e.(map[string]any); ok {
				items = append(items, item{Domain: toString(m["domain"]), RoleID: toString(m["role_id"])})
			}
		}
		// build set value
